...
{{-- الشات بوت الذكي للأسئلة الشائعة — أسفل اليمين كما في المرجع --}}
<div x-data="chatbot(@js(route('chatbot')), @js(setting('chatbot.welcome', 'أهلاً بيك! اسألني عن الاشتراك أو الدفع أو المنصة.')))"
    ...
    <div x-show="open" x-cloak x-transition
         class="absolute bottom-[4.5rem] right-0 flex h-[26rem] w-[19rem] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-night-700 dark:bg-night-900 sm:w-80">
        ...
        <div x-ref="box" class="flex-1 space-y-2.5 overflow-y-auto p-3">
            <template x-for="(m, i) in messages" :key="i">
                <div :class="m.from === 'me' ? 'mr-auto bg-brand-500 text-white' : 'ml-auto bg-slate-100 text-slate-700 dark:bg-night-800 dark:text-slate-200'"
                     class="max-w-[85%] whitespace-pre-line rounded-2xl px-3.5 py-2 text-sm font-medium leading-6"
                     x-text="m.text"></div>
            </template>
        ...
        <form @submit.prevent="send()" class="flex items-center gap-2 border-t border-slate-100 p-2.5 dark:border-night-800">
            <input type="text" x-model="draft" placeholder="اكتب سؤالك هنا..." class="input py-2">
